...
fid = fopen('fir_output_sinewave1_q214.txt','r');
v1_ip = fscanf(fid,'%d',N)'; fclose(fid);

fid = fopen('fir_output_sinewave2_q214.txt','r');
v2_ip = fscanf(fid,'%d',N)'; fclose(fid);

fid = fopen('fir_output_sinewave3_q214.txt','r');
v3_ip = fscanf(fid,'%d',N)'; fclose(fid);
...
plot(t,output1_dir); hold on
plot(t,vout1_ip,'--')
title('950 Hz FIR IP Output')
legend('MATLAB','FIR IP')
xlim([0 0.05])

subplot(3,1,2)
plot(t,output2_dir); hold on
plot(t,vout2_ip,'--')
title('1100 Hz FIR IP Output')
legend('MATLAB','FIR IP')
xlim([0 0.05])
