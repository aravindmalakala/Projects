...
reg  signed [15:0] x_pipe   [0:TAPS-2][0:1]; // two regs between each stage (top rail)
reg  signed [39:0] sum_pipe [0:TAPS-2];      // one sum reg between each stage (bottom rail)
reg  signed [39:0] mul_reg  [0:TAPS-1];      // registered multiplier outputs
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        // Multipliers
        mul_reg[0] <= x_in * h[0]; // stage 0: x_in feeds directly, no input register
        for (i = 1; i < TAPS; i = i + 1)
            mul_reg[i] <= x_pipe[i-1][1] * h[i];

        // Shift register (top rail)
        x_pipe[0][0] <= x_in;
        x_pipe[0][1] <= x_pipe[0][0];
        for (i = 1; i < TAPS-1; i = i + 1) begin
            x_pipe[i][0] <= x_pipe[i-1][1];
            x_pipe[i][1] <= x_pipe[i][0];
        end

        // Accumulator pipeline (bottom rail)
        sum_pipe[0] <= mul_reg[0];
        for (i = 1; i < TAPS-1; i = i + 1)
            sum_pipe[i] <= sum_pipe[i-1] + mul_reg[i];

        // Register output
        y_out <= sum_pipe[TAPS-2] + mul_reg[TAPS-1];
    end
end
